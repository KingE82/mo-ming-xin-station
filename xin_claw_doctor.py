...
SYNDROME_KNOWLEDGE = {
    ...
    "肝血虚证": {
        "organs": ["肝"],
        "nature": "虚证",
        "symptoms": ["头晕", "目眩", "面色苍白", "爪甲不荣", "肢体麻木", "月经量少", "舌淡", "脉细","目涩","视物模糊"],
        "principle": "补肝养血",
    ...
    "湿热蕴脾": {
        "organs": ["脾", "肝"],
        "nature": "实证",
        "symptoms": ["脘腹胀闷", "口苦", "纳呆", "便溏不爽", "舌红苔黄腻", "脉滑数","皮肤瘙痒","湿疹"],
        "principle": "清热利湿，化浊和中",
    ...
    "胃阴虚证": {
        "organs": ["胃"],
        "nature": "虚证",
        "symptoms": ["胃脘隐痛", "饥不欲食", "口干", "干呕", "便秘", "舌红少津", "脉细数","胃脘痛","反酸","嗳气"],
        "principle": "养阴益胃",
...
def differentiate(symptoms: List[str], tongue: str = "", pulse: str = "", voice: str = "") -> Dict:
    """辨证引擎"""
    if not symptoms:
    ...
    # ═══ 口语症状 → 证候词 归一化映射（让勾选的口语词能匹配证型库的文言证候） ═══
    COLLOQUIAL_MAP = {
        '失眠': '失眠', '嗜睡': '嗜睡', '多梦': '多梦', '心悸': '心悸', '心烦': '心烦',
        '胸闷': '胸闷', '健忘': '健忘', '烦躁': '烦躁易怒', '焦虑': '焦虑不安',
        '易惊': '易惊', '悲伤欲哭': '悲伤欲哭', '心神不宁': '心神不宁',
        '急躁易怒': '急躁易怒', '情绪抑郁': '情绪抑郁', '胁肋胀痛': '胁肋胀痛',
        '头晕目眩': '头晕目眩', '目赤': '目赤', '手足麻木': '肢体麻木', '肢体麻木': '肢体麻木',
        '眼干涩': '目涩', '视物模糊': '视物模糊', '耳鸣': '耳鸣', '抽筋': '筋惕肉瞤',
        '震颤': '肢体震颤',
        '食欲不振': '纳呆', '消谷善饥': '消谷善饥', '暴食': '暴饮暴食', '腹胀': '脘腹胀满',
        '便溏': '便溏', '便秘': '便秘', '乏力': '神疲乏力', '恶心': '恶心', '呕吐': '呕吐',
        '反酸': '嗳腐吞酸', '嗳气': '嗳气', '胃痛': '胃脘痛', '腹痛': '腹痛',
        '腹泻': '泄泻', '口淡无味': '口淡', '口黏腻': '口中黏腻', '口臭': '口气秽臭',
        '舌苔厚腻': '苔厚腻', '消瘦': '形体消瘦',
        '咳嗽': '咳嗽', '气喘': '气喘', '气短': '气短', '痰多': '痰多',
        '自汗': '自汗', '易感冒': '易感冒', '鼻塞': '鼻塞', '流清涕': '流清涕',
        '流黄涕': '流黄涕', '咽干': '咽干', '咽痛': '咽痛', '声音嘶哑': '声音嘶哑',
        '咳痰黄稠': '黄痰', '咳痰清稀': '清稀痰', '干咳': '干咳', '咯血': '咯血',
        '皮肤瘙痒': '皮肤瘙痒', '湿疹': '湿疹',
        '腰膝酸软': '腰膝酸软', '畏寒': '畏寒', '怕热': '怕热', '五心烦热': '五心烦热',
        '盗汗': '盗汗', '夜尿多': '夜尿多', '尿频': '尿频', '尿急': '尿急', '尿痛': '尿痛',
        '小便清长': '小便清长', '小便黄': '小便短黄', '遗精': '遗精', '脱发': '发脱',
        '牙齿松动': '齿摇',
        '头痛': '头痛', '口干': '口干', '口苦': '口苦', '面色淡白': '面色淡白',
        '面色萎黄': '面色萎黄', '浮肿': '水肿', '四肢冰凉': '肢冷', '手足心热': '手足心热',
        '汗多': '汗出过多', '汗少': '无汗', '面色潮红': '面色潮红', '面色晦暗': '面色晦暗',
        '眼圈发黑': '面色晦暗', '背痛': '背痛', '腰痛': '腰痛', '关节痛': '关节疼痛',
        '身重': '身重', '嗜卧': '嗜卧', '口唇干裂': '口唇干燥', '皮肤干燥': '皮肤干燥',
    }
    normalized = set()
    for s in list(symptom_set):
        mapped = COLLOQUIAL_MAP.get(s)
        if mapped:
            normalized.add(mapped)
    symptom_set.update(normalized)
    ...
    if tongue or pulse or voice:
        scores = apply_four_diag_bonus(scores, tongue, pulse, voice)
...
FOUR_DIAG_KNOWLEDGE = {
    "心气虚证": {"tongue": {"body": "舌淡", "coat": "苔白"}, "pulse": "脉虚弱/脉沉弱", "voice": ["语声低微", "少气懒言"],
        "classic_formulas": [
    ...
    "脾气虚证": {"tongue": {"body": "舌淡", "coat": "苔白"}, "pulse": "脉缓弱", "voice": ["语声低微", "少气懒言"],
        "classic_formulas": [
    ...
    "肺气虚证": {"tongue": {"body": "舌淡", "coat": "苔白"}, "pulse": "脉虚弱", "voice": ["语声低微", "少气懒言", "咳声重浊"],
        "classic_formulas": [
    ...
    "风寒束肺证": {"tongue": {"body": "舌淡红", "coat": "苔薄白"}, "pulse": "脉浮紧", "voice": ["咳声重浊", "声重浊"],
        "classic_formulas": [
    ...
    "肾阳虚证": {"tongue": {"body": "舌淡胖", "coat": "苔白"}, "pulse": "脉沉迟", "voice": ["语声低微"],
        "classic_formulas": [
...
def apply_four_diag_bonus(scores: List[Dict], tongue_input: str, pulse_input: str, voice_input: str = "") -> List[Dict]:
    """舌象/脉象加权"""
    if not tongue_input and not pulse_input and not voice_input:
        return scores
    for s in scores:
        bonus = 0.0
        ...
        if pulse_input and diag.get("pulse"):
            ...
            for pn in pulse_names:
                if pn in diag["pulse"]:
                    bonus += 0.1
        if voice_input and diag.get("voice"):
            voice_names = re.split(r'[,，、;；\s/]+', voice_input)
            for vn in voice_names:
                if vn and vn in diag.get("voice", []):
                    bonus += 0.1
...
SYNDROME_WESTERN_MAP = {
    "心气虚证": ["慢性心功能不全", "心脏神经症", "心肌炎恢复期"], "voice": ["语声低微", "少气懒言"],
    "心血虚证": ["贫血", "神经衰弱", "心脏神经症"],
    "心阴虚证": ["更年期综合征", "甲状腺功能亢进", "神经官能症"],
    "心阳虚证": ["慢性心力衰竭", "病态窦房结综合征", "甲状腺功能减退"],
    "心火亢盛证": ["口腔溃疡", "失眠症", "高血压"], "voice": ["语声高亢", "口气秽臭"],
    "心脉痹阻证": ["冠心病", "心肌梗死", "心绞痛", "动脉粥样硬化"],
    "心脾两虚证": ["贫血", "神经衰弱", "功能性消化不良", "慢性疲劳综合征"],
    "肝气郁结证": ["抑郁症", "焦虑症", "功能性消化不良", "乳腺增生", "慢性胃炎"], "voice": ["善太息"],
    "肝火上炎证": ["高血压", "偏头痛", "结膜炎", "三叉神经痛"], "voice": ["语声高亢", "口气秽臭"],
    "肝血虚证": ["贫血", "干眼症", "慢性肝病", "月经不调"],
    "肝阳上亢证": ["高血压病", "脑血管意外风险", "偏头痛", "耳鸣"],
    "肝风内动证": ["帕金森病", "震颤", "高血压脑病", "脑血管意外"],
    "肝火犯肺证": ["慢性咳嗽", "支气管炎", "咽炎"],
    "肝肾阴虚证": ["高血压", "慢性肝炎", "更年期综合征", "干眼症"],
    "脾气虚证": ["慢性胃炎", "功能性消化不良", "肌无力", "慢性疲劳综合征"], "voice": ["语声低微", "少气懒言"],
    "脾阳虚证": ["慢性肠炎", "肠易激综合征", "甲状腺功能减退"],
    "脾不统血证": ["血小板减少性紫癜", "功能性子宫出血", "凝血功能障碍"],
    "脾肾阳虚证": ["慢性腹泻", "甲状腺功能减退", "慢性肾病", "肾上腺皮质功能减退"],
    "寒湿困脾证": ["慢性胃肠炎", "消化不良", "水肿待查"],
    "湿热蕴脾": ["慢性肝炎", "胆囊炎", "胃炎伴幽门螺杆菌感染"], "voice": ["口气秽臭", "口气酸腐"],
    "暑湿困脾": ["夏季热", "胃肠型感冒", "中暑"], "voice": ["声重浊", "口气秽臭"],
    "肺气虚证": ["慢性阻塞性肺疾病", "肺气肿", "反复呼吸道感染"], "voice": ["语声低微", "少气懒言", "咳声重浊"],
    "肺阴虚证": ["肺结核待排查", "慢性咽炎", "干燥综合征"],
    "风寒束肺证": ["上呼吸道感染", "急性支气管炎"], "voice": ["咳声重浊", "声重浊"],
    "风热犯肺": ["上呼吸道感染", "急性扁桃体炎", "咽炎"], "voice": ["咳声清脆", "语声高亢"],
    "痰湿阻肺证": ["慢性支气管炎", "慢性阻塞性肺疾病", "支气管扩张"], "voice": ["咳声重浊", "痰气腥臭"],
    "肺热壅盛证": ["肺炎", "急性支气管炎", "肺脓肿待排除"], "voice": ["咳声清脆", "语声高亢", "口气秽臭"],
    "燥邪伤肺": ["急性咽炎", "干燥综合征", "过敏性咳嗽"], "voice": ["咳声清脆"],
    "肺肾阴虚证": ["肺结核", "慢性咽炎", "干燥综合征", "慢性肾病"],
    "肾阳虚证": ["甲状腺功能减退", "肾上腺皮质功能减退", "慢性肾病", "性功能障碍"], "voice": ["语声低微"],
    "肾阴虚证": ["高血压", "糖尿病", "更年期综合征", "慢性肾病"],
    "肾精不足证": ["阿尔茨海默病", "骨质疏松", "早衰综合征"],
    "肾气不固证": ["前列腺增生", "尿失禁", "遗精", "慢性肾病"],
    "肾不纳气证": ["慢性阻塞性肺疾病", "肺气肿", "支气管哮喘"],
    "胃阴虚证": ["慢性萎缩性胃炎", "干燥综合征", "糖尿病性胃轻瘫"], "voice": ["语声高亢"],
    "食滞胃脘证": ["功能性消化不良", "急性胃炎", "暴食症"], "voice": ["口气酸腐", "口气秽臭"],
    "胃寒证": ["慢性胃炎", "胃痉挛", "胃肠功能紊乱"], "voice": ["少气懒言"],
    "大肠湿热证": ["急性肠炎", "溃疡性结肠炎", "细菌性痢疾"],
    "膀胱湿热证": ["尿路感染", "膀胱炎", "前列腺炎"],
    "上热下寒": ["高血压", "慢性胃炎", "植物神经功能紊乱"],
    "阴虚火旺": ["甲状腺功能亢进", "更年期综合征", "结核病待排除"], "voice": ["语声高亢"],
    "表寒证": ["上呼吸道感染", "普通感冒"],
    "表热证": ["上呼吸道感染", "流行性感冒", "急性咽炎"],
    "里寒证": ["胃肠功能紊乱", "肠易激综合征"],
    "里热证": ["急性胃肠炎", "发热待查"],
    "风寒束表": ["上呼吸道感染", "普通感冒"], "voice": ["声重浊"],
    "风寒袭肺": ["急性支气管炎", "上呼吸道感染"], "voice": ["咳声重浊", "声重浊"],
}
